...
<div class="displays allResults new_data" id="revenue_report">
    <h2>Officer Collections for today</h2>
    <hr>
    <div class="search">
    ...
    <table id="data_table" class="searchTable">
        ...
        <tbody>
            <?php
                ...
                $details = $get_users->fetch_details_curdateGro1con('deposits', 'post_date', 'store', $store, 'posted_by');
                if(gettype($details) === 'array'){
                foreach($details as $detail):
                    //get posted by
                    $psts = $get_users->fetch_details_cond('users', 'user_id', $detail->posted_by);
                    if(is_array($psts)){
                        foreach($psts as $ps){
            ...
            <tr>
                ...
                <td>
                    ...
                    <?php }else{?>
                    <a href="javascript:void(0)" style="color:var(--otherColor)" onclick="showPage('officer_collection.php?cashier=<?php echo $detail->posted_by?>')"><?php echo $cashier?></a>
                    <?php }?>
                </td>
                <td>
                    <?php
                        $get_cash = new selects();
                        $cashs = $get_cash->fetch_sum_curdate2Con('deposits', 'amount', 'date(post_date)', 'payment_mode', 'Cash', 'posted_by', $detail->posted_by);
                ...
                <td>
                    <?php
                        $get_pos = new selects();
                        $poss = $get_pos->fetch_sum_curdate2Con('deposits', 'amount', 'date(post_date)', 'payment_mode', 'POS', 'posted_by', $detail->posted_by);
                        foreach($poss as $pos){
                ...
                <td>
                    <?php
                        $get_tf = new selects();
                        $trfs = $get_tf->fetch_sum_curdate2Con('deposits', 'amount', 'date(post_date)', 'payment_mode', 'Transfer', 'posted_by', $detail->posted_by);
                        foreach($trfs as $trf){
                ...
                <td style="color:red;">
                    <?php
                        ...
                        $amounts = $get_total->fetch_sum_curdateCon('deposits', 'amount', 'post_date', 'posted_by', $detail->posted_by);
            ...
            <tr>
                ...
                <td style="color:green; font-size:1rem;">
                    <?php
                        $get_total = new selects();
                        $totals = $get_cash->fetch_sum_curdate2Con('deposits', 'amount', 'post_date', 'payment_mode', 'Cash', 'store', $store);
                ...
                <td style="color:green; font-size:1rem;">
                    <?php
                        $get_total = new selects();
                        $totals = $get_cash->fetch_sum_curdate2Con('deposits', 'amount', 'post_date', 'payment_mode', 'POS', 'store', $store);
                        foreach($totals as $total){
                            echo "₦".number_format($total->total, 2);
                        }
                    ?>
                ...
                <td style="color:green; font-size:1rem;">
                    <?php
                        $get_total = new selects();
                        $totals = $get_cash->fetch_sum_curdate2Con('deposits', 'amount', 'post_date', 'payment_mode', 'Transfer', 'store', $store);
                        foreach($totals as $total){
                            echo "₦".number_format($total->total, 2);
                        }
                    ?>
