<?php
    ...
    $details = $get_revenue->fetch_details_2date2Con('deposits', 'date(post_date)', $from, $to, 'payment_mode', 'Transfer', 'store', $store);
    $n = 1;  
?>
<h2>POS Payments Between '<?php echo date("jS M, Y", strtotime($from)) . "' and '" . date("jS M, Y", strtotime($to))?>'</h2>
    <hr>
    <div class="search">
...
<?php
    ...
    $amounts = $get_total->fetch_sum_2date2Cond('deposits', 'amount', 'date(post_date)', 'store', 'payment_mode', $from, $to, $store, 'Transfer');
