<?php
    ...
    $details = $get_revenue->fetch_details_2dateCon('finance_cost', 'store', 'date(post_date)', $from, $to,  $store);
    $n = 1;
?>
<h2>Finance Costs Transactions from '<?php echo date("jS M, Y", strtotime($from)) . "' to '" . date("jS M, Y", strtotime($to))?>'</h2>
    <hr>
    <div class="search">
        <input type="search" id="searchRevenue" placeholder="Enter keyword" onkeyup="searchData(this.value)">
        <a class="download_excel" href="javascript:void(0)" onclick="convertToExcel('data_table', 'Finance Costs Transactions')"title="Download to excel"><i class="fas fa-file-excel"></i></a>
    </div>
...
?>
    <div class="all_modes">
        <?php
            ...
            $cashs = $get_cash->fetch_sum_2dateCond('finance_cost', 'amount', 'store', 'date(post_date)', $from, $to, $store);
